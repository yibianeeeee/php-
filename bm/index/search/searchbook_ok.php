...
$a=$_POST["search"];
$sql="select * from book where name='$a'";
...
<head>
    ...
    <link rel="stylesheet" href="../delbook/css/reset.min.css">


    <link rel="stylesheet" href="../delbook/css/style.css">
...
<section>
    ...
    <div class="tbl-content">
        <table cellpadding="0" cellspacing="0" border="0">
            <tbody>
            <?php
            $pagesize = 3 ;								//2.每页显示记录数
            $sqlstr = "select * from book where name='$a' order by id";//1.定义查询语句
            ...
            while ($rows = mysqli_fetch_array($result)){			//1.2.循环输出查询结果
                ...
                <td>
                    <a href =../borrow/borrowbook_ok.php?action=borrow&id=<?php echo $rows[0]?>>借阅</td>
                </td>
                </tr>
                <?php
                $i++;
...
<script src='../delbook/js/jquery.min.js'></script>

<script  src="../delbook/js/index.js"></script>

</body>
</html>
<?php
}else{
    echo"<script>alert('无此书籍');history.go(-1);</script>";
}
